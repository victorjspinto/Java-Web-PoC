/// <reference path="../definitionFiles/angular/angular.d.ts" />
/// <reference path="../definitionFiles/jquery/jquery.d.ts" />
/// <reference path="../definitionFiles/requirejs/require.d.ts" />


//grunt-start
/// <reference path="config/appConfig.ts" />
/// <reference path="config/appRoutes.ts" />
/// <reference path="controller/StudentListController.ts" />
/// <reference path="controller/base/AbstractController.ts" />
/// <reference path="entity/Student.ts" />
/// <reference path="entity/VO/Telephone.ts" />
/// <reference path="entity/base/BaseEntity.ts" />
/// <reference path="main.ts" />
/// <reference path="util/ArrayUtilities.ts" />
/// <reference path="util/DependencyResolver.ts" />
/// <reference path="util/UploadManager.ts" />
//grunt-end